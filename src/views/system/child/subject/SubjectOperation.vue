<template>
  <div class="container fixed">
    ...
    <el-form class="ruleForm center" :model="ruleForm" ref="ruleForm" :rules="rules">
      <el-form-item label="学科" label-width="80px" prop="name" class="item">
        <el-input clearable v-model="ruleForm.name" placeholder="请输入学科"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    //学科添加
    add() {
      this.http.post(this.api.subject.add, this.ruleForm).then( res => {
        let { code, msg } = res
        if(code == 1) {
    ...
    //学科修改
    modify() {
      this.http.put(this.api.subject.update, this.ruleForm).then( res => {
        let { code, msg } = res
        if(code == 1) {
