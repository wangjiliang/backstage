<template>
  <div>
    <el-table class="tableData"  ref="multipleTable" highlight-current-row :data="tableData"
      ...
      <el-table-column prop="name" label="资讯分类" show-overflow-tooltip></el-table-column>
    ...
    <CategoryOperation v-if="showModal || isAdd" :row="row" @hide="hide" @confirm="handleConfirm"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    fetchData(pageNum, pageSize) {
      ...
      this.http.get(this.api.cms.listpage, params).then( res => {
    ...
    changeStatus(row) {
      ...
      this.http.put(this.api.cms.update, params).then( res => {
