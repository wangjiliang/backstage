<template>
  <div>
    ...
    <GradeOperation v-if="showModal || isAdd" :row="row" @hide="hide" @confirm="handleConfirm" />
  </div>
</template>
...
import GradeOperation from './GradeOperation'

export default {
  mixins: [getData, period, operation],
  components: {GradeOperation },
  mounted () {
    this.fetchData()
  },
  methods: {
    fetchData(pageNum, pageSize) {
      ...
      this.http.get(this.api.grade.list, params).then( res => {
    ...
    changeStatus(row) {
      ...
      this.http.put(this.api.grade.update, params).then( res => {
